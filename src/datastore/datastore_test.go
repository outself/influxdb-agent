...
3.6 million values split across 180 time series took up about 121MB on disk
1 million log lines written like this took 36MB on disk
...
// 	startTime := time.Now().Unix()/86400*86400 + 1
// 	pointsWritten := 0
// 	logSeriesName := host + ".logs./home/ubuntu/ruby_example/test_log.txt"
// 	for i := 0; i < 500000; i++ {
// 		v := float64(1.0)
// 		l := strconv.FormatInt(rand.Int63(), 10)
// 		lines := "liens here lkjasdflkjsjlka lksd and othe rstuff " + l + "\nlkasjd" + l + " fljkadsf wlkjwlekj  asdfoiw"
// 		err := ds.WritePoints(database, logSeriesName, []*protocol.Point{&protocol.Point{Value: &v, Time: &startTime, Context: &lines}})
// 		c.Assert(err, IsNil)
// 		pointsWritten += 1
// 		startTime += 1
// 	}
// 	fmt.Printf("Lines written to test db: %d", pointsWritten*2)
// }
